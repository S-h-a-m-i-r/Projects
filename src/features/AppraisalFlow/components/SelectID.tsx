...
import { setAppraisal } from "../../AppraisalFlow/ducks/appraisalFlow.reducer";
...
const RoleTeam: React.FC<SelectTeamProps> = ({ initialValue }) => {
	...
	const { selected_assign_id } = payload || { selected_assign_id: "" };
	...
	useEffect(() => {
		if (payload.assign_type_name === ROLES) {
			const fetchDesignations = async () => {
				await dispatch(getEmployeeDesignation());
			};

			fetchDesignations();
		} else if (payload.assign_type_name === TEAMS) {
			const fetchTeams = async () => {
				await dispatch(getTeamList());
			};

			fetchTeams();
		} else if (payload.assign_type_name === INDIVIDUALS) {
	...
	}, [payload.assign_type_name, dispatch]);

	useEffect(() => {
		if (payload.assign_type_name === ROLES && nameList.designations.length > 0) {
		...
		} else if (payload.assign_type_name === TEAMS && teams.projects) {
	...
	}, [nameList.designations, teams.projects, selectedValue, payload.assign_type_name]);

	return (
		<>
			<Stack mt={10}>
				{payload.assign_type_name === ROLES && (
					<Stack>
				...
				{payload.assign_type_name === TEAMS && teams && (
					<Stack>
						<Typography>Select</Typography>
						<Select
							size="small"
				...
				{payload.assign_type_name === INDIVIDUALS && emplist.empdetails && (
					<Stack>
						<Typography>Select</Typography>
						<Select
							size="small"
